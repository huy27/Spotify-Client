...
import { SetSong } from "../reducer/SongSlice";
...
export default function Playing() {
  ...
  useEffect(() => {
    ...
    return player.current.audio.current.removeEventListener(
      "volumechange",
      (e) => {
        dispatch(SetVolume(`${(e.target.volume * 100).toFixed(0)}`));
      }
  ...
  const handleClickNext = () => {
    ...
    dispatch(SetSong({ idSong: songs[next + 1]?.id, songs: songs }));
  };

  const handleClickPre = () => {
    const previous = songs.findIndex((s) => s.id === song.id);
    dispatch(SetSong({ idSong: songs[previous - 1]?.id, songs: songs }));
  };
    ...
    <div>
      <AudioPlayer
        ...
        customAdditionalControls={[
          RHAP_UI.LOOP,
          <button
            className="button_lyrics"
            onClick={() => handleShowLyric(!isShowLyric, song?.lyric)}
          >
        ...
        customVolumeControls={[
          RHAP_UI.VOLUME,
          <div className="white-text" key={2}>
            &nbsp;&nbsp;{audio.volume}
          </div>,
        ]}
      />
