...
const ListSongs = ({ height }) => {
  ...
  useEffect(() => {
    if (localStorage.getItem("Favorite")) {
      dispatch(
        addListFavoriteSong(JSON.parse(localStorage.getItem("Favorite")))
      );
    }
  }, []);
  ...
  const handlePlaySong = (idSong) => {
    setidSong(idSong);
    dispatch(SetSong({ idSong: idSong, songs: DataSongs }));
    dispatch(
      ShowLyric({
        isShow: lyric.isShow,
        lyric: DataSongs.find((s) => s.id === idSong)?.lyric,
      })
    );
  };
                ...
                <td className="text-center">
                  <i
                    ...
                    className={`fa-solid fa-star ${
                      favorite.find((f) => f.id === song.id) && "fa-star-active"
                    }`}
                  ></i>
                </td>
                <td>{song.name}</td>
